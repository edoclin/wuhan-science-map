...
import { listBaseHall, deleteBaseHallByIds, getBaseHallConditions, getTableColumns, putBaseHall } from 'src/api/hall'
import BaseHallFormVue from './Form.vue'
...
getBaseHallConditions().then(res => {
...
const fetchListData = (postHandler) => {
  ...
  listBaseHall(page.current, page.size, { ...queryParam }).then(res => {
...
const deleteSelected = () => {
  ...
  deleteBaseHallByIds({ ids }).then(res => {
    ElMessage({
      type: 'success',
      message: res.data
    })
...
const onEdit = (record) => {
  bus.emit('edit-item', {
    record,
    component: BaseHallFormVue,
    title: '展厅编辑',
    name: record.id
  })
}

bus.on('update-hall-table', () => fetchListData())
</script>
<style>
