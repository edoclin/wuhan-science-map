...
import { listZhihuiItem, deleteZhihuiItemByIds, getZhihuiItemConditions, getTableColumns, putZhihuiItem } from 'src/api/zhihui-item'
import ZhihuiItemFormVue from './Form.vue'
...
getZhihuiItemConditions().then(res => {
  conditions.map = res.data
})

const queryParam = reactive({
  isAsc: true,
  orderColumns: ['displayIndex'],
  conditions: []
})
...
const fetchListData = (postHandler) => {
  ...
  listZhihuiItem(page.current, page.size, { ...queryParam }).then(res => {
...
const deleteSelected = () => {
  ...
  deleteZhihuiItemByIds({ ids }).then(res => {
    ElMessage({
      type: 'success',
      message: res.data
    })
...
const onEdit = (record) => {
  bus.emit('edit-item', {
    record,
    component: ZhihuiItemFormVue,
    title: '编辑',
    name: record.id
  })
}

bus.on('update-zhihuiItem-table', () => fetchListData())
</script>
<style>
