<template>
  <el-form :rules="rules" :model="form" label-width="120px" ref="formRef">
    ...
    <el-form-item label="讲师照片" prop="lecturerPicture">
    ...
    <el-form-item label="讲师介绍" prop="descRichText">
...
import { postLecturer, putLecturer } from 'src/api/lecturer'
...
const handleChangeFileUpload = (uploadFile) => {
  ...
  fileList.value = []
  sliceUploadFile(uploadFile.raw, 'lecturer-picture').then(data => {
    form['lecturerPicture'] = data['Key']
    ElMessage({
      type: 'success',
      message: '上传成功'
    })
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (props.data && JSON.stringify(props.data) !== '{}') {
        await putLecturer(form).then(res => {
          ElMessage({
            type: 'success',
            message: res.data,
          })
        ...
        await postLecturer(form).then(res => {
          ElMessage({
            type: 'success',
            message: res.data,
          })
      ...
      fileList.value = []
      bus.emit('update-lecturer-table')
    }
  })
}
...
if (props.data && JSON.stringify(props.data) !== '{}') {
  ...
  form['lecturerName'] = props.data['lecturerName']
  form['baseId'] = props.data['baseId']
  form['lecturerPicture'] = props.data['lecturerPicture']
  fileList.value = [{
    name: '',
    url: props.data['lecturerPictureUrl']
  }]
}
