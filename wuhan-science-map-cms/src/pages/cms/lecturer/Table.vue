...
import { listLecturer, deleteLecturerByIds, getLecturerConditions, getTableColumns, putLecturer } from 'src/api/lecturer'
import LecturerFormVue from './Form.vue'
...
getLecturerConditions().then(res => {
...
const fetchListData = (postHandler) => {
  ...
  listLecturer(page.current, page.size, { ...queryParam }).then(res => {
...
const deleteSelected = () => {
  ...
  deleteLecturerByIds({ ids }).then(res => {
    ElMessage({
      type: 'success',
      message: res.data
    })
...
const onEdit = (record) => {
  bus.emit('edit-item', {
    record,
    component: LecturerFormVue,
    title: '讲师编辑',
    name: record.id
  })
}

bus.on('update-lecturer-table', () => fetchListData())
</script>
<style>
