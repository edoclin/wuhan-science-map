    ...
    <el-table v-loading="tableData.loading" @sort-change='sortTable' border table-layout="auto" stripe
      ...
      <el-table-column fixed="right" label="操作">
        ...
        <template #default="scope">
          ...
          <el-button type="primary" link @click="resultDisplay(scope.row)" size="small">成果展示</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import {
  listUserRegisterCourseByCourseId,
  deleteUserRegisterCourseByIds,
  getUserRegisterCourseConditions,
  getTableColumns,
} from 'src/api/user-register-course'
import UserRegisterCourseFormVue from './Form.vue'
import UserResultDisplayVue from '../result-display/Table.vue'
...
getUserRegisterCourseConditions().then(res => {
...
const fetchListData = (postHandler) => {
  ...
  listUserRegisterCourseByCourseId(page.current, page.size, { ...queryParam }).then(res => {
...
const deleteSelected = () => {
  ...
  deleteUserRegisterCourseByIds({ ids }).then(res => {
    ElMessage({
      type: 'success',
      message: res.data
    })
...
const resultDisplay = (record) => {
  bus.emit('edit-item', {
    record,
    component: UserResultDisplayVue,
    title: '用户课程成果展示',
    name: record.id
  })
}
const onEdit = (record) => {
  bus.emit('edit-item', {
    record,
    component: UserRegisterCourseFormVue,
    title: '编辑',
    name: record.id
  })
}

bus.on('update-user-register-course-table', () => fetchListData())

const props = defineProps({
  data: {}
})
if (props.data && JSON.stringify(props.data) !== '{}') {
  queryParam['conditions'].push(
      {
        'column': 'userId',
        'value': props.data['id'],
        'isAnd': true
