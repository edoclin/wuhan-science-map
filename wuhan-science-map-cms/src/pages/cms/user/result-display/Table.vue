...
import { listUserUploadCourse, deleteUserUploadCourseByIds, getUserUploadCourseConditions, getTableColumns, putUserUploadCourse } from 'src/api/user-upload-course'
import UserUploadCourseFormVue from './Form.vue'
...
getUserUploadCourseConditions().then(res => {
...
const fetchListData = (postHandler) => {
  ...
  listUserUploadCourse(page.current, page.size, { ...queryParam }).then(res => {
...
const deleteSelected = () => {
  ...
  deleteUserUploadCourseByIds({ ids }).then(res => {
    ElMessage({
      type: 'success',
      message: res.data
    })
...
const onEdit = (record) => {
  bus.emit('edit-item', {
    record,
    component: UserUploadCourseFormVue,
    title: '成果编辑',
    name: record.id
  })
}

bus.on('update-user-upload-course-table', () => fetchListData())
</script>
<style>
