...
import { listLink, deleteLinkByIds, getLinkConditions, getTableColumns, putLink } from 'src/api/link'
import LinkFormVue from './Form.vue'
...
const fetchListData = (postHandler) => {
  ...
  listLink(page.current, page.size, { ...queryParam }).then(res => {
...
bus.on('update-link-table', () => fetchListData())
</script>
<style>
