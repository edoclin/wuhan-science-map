...
import { listZhihuiTrip, deleteZhihuiTripByIds, getZhihuiTripConditions, getTableColumns, putZhihuiTrip } from 'src/api/zhihui'
import ZhihuiTripFormVue from './Form.vue'
...
getZhihuiTripConditions().then(res => {
...
const fetchListData = (postHandler) => {
  ...
  listZhihuiTrip(page.current, page.size, { ...queryParam }).then(res => {
...
const deleteSelected = () => {
  ...
  deleteZhihuiTripByIds({ ids }).then(res => {
    ElMessage({
      type: 'success',
      message: res.data
    })
...
const onEdit = (record) => {
  bus.emit('edit-item', {
    record,
    component: ZhihuiTripFormVue,
    title: '智慧行编辑',
    name: record.id
  })
}

bus.on('update-zhihui-table', () => fetchListData())
</script>
<style>
