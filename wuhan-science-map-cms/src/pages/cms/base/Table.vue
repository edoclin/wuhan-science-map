    ...
    <el-table v-loading="tableData.loading" @sort-change='sortTable' border table-layout="auto" stripe
      ...
      <el-table-column fixed="right" label="操作">
        ...
        <template #default="scope">
          ...
          <el-button type="primary" link @click="copyId(scope.row)" size="small">复制编号</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { listScienceBase, deleteScienceBaseByIds, getScienceBaseConditions, getTableColumns } from 'src/api/science-base'
import ScienceBaseFormVue from './Form.vue'
...
getScienceBaseConditions().then(res => {
...
const fetchListData = (postHandler) => {
  ...
  listScienceBase(page.current, page.size, { ...queryParam }).then(res => {
...
const deleteSelected = () => {
  ...
  console.log(ids)
  deleteScienceBaseByIds({ ids }).then(res => {
    ElMessage({
      type: 'success',
      message: res.data
    })
...
bus.on('update-base-table', () => fetchListData())
</script>
<style>
