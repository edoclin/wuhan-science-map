...
import { listNews, deleteNewsByIds, getNewsConditions, getTableColumns, putNews } from 'src/api/news'
import NewsFormVue from './Form.vue'
...
const fetchListData = (postHandler) => {
  ...
  listNews(page.current, page.size, { ...queryParam }).then(res => {
...
const onEdit = (record) => {
  bus.emit('edit-item', {
    record,
    component: NewsFormVue,
    title: '资讯编辑',
    name: record.id
  })
}

bus.on('update-goods-table', () => fetchListData())
</script>
<style>
