...
import { list$module$, delete$module$ByIds, get$module$Conditions, getTableColumns, put$module$ } from 'src/api/$api_file$'
import $module$FormVue from './Form.vue'
...
get$module$Conditions().then(res => {
...
const fetchListData = (postHandler) => {
  ...
  list$module$(page.current, page.size, { ...queryParam }).then(res => {
...
const deleteSelected = () => {
  ...
  delete$module$ByIds({ ids }).then(res => {
    ElMessage({
      type: 'success',
      message: res.data
    })
...
const onEdit = (record) => {
  bus.emit('edit-item', {
    record,
    component: $module$FormVue,
    title: '$title_name$编辑',
    name: record.id
  })
}

bus.on('update-$api_file$-table', () => fetchListData())
</script>
<style>
