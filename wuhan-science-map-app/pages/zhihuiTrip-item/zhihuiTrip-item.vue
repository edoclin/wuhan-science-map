	...
	<view>
		...
		<view class="travel-main">
			<view class="header-wrap">
				...
				<view class="header-item">{{tripName}}</view>
			</view>
			
			<view>
				<u-skeleton :loading="skeleton" :animate="true" :rows="20" :title="false">
					<u-empty v-if="zhihuiTripItemList.length === 0" mode="list"
						icon="http://cdn.uviewui.com/uview/empty/list.png" text="来早了...">
					</u-empty>
					<view v-else>
						<view class="goods-item-wrap" @click="toTripItemDetail(item.id)"
							v-for="(item, index) in zhihuiTripItemList" :key="index">
...
<script setup>
	...
	const zhihuiTripItemList = ref([])
	const skeleton = ref(false)
	let tripName = ref()
	onLoad((option) => {
		tripName.value = option.name
		zhihuiTripListById(option['id']).then(res => {
			setTimeout(()=>{
				skeleton.value = false
			},300)
			zhihuiTripItemList.value = res.data
		})
	})
	
	const toTripItemDetail = (id) =>{
		uni.navigateTo({
			url: `/pages/zhihuiTrip-item-detail/zhihuiTrip-item-detail?id=${id}`
		})
	}
